...
var DropDown = Class.extend({
	...
	,setValue : function(id, name){
		...
		if (id !=-1 && this.items && this.items.length){																		// list available -> select item on list
			...
			if (!name) name = this.mapName(this.conf.fieldName, this.selectedItem);
	...
	,disable : function(reset){ if (reset) this.setValue(); this.el.addClass('dropdown-disabled'); }
	,enable : function(reset){ if (reset) this.setValue(); this.el.removeClass('dropdown-disabled'); }
